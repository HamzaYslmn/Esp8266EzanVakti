...
void setup() {
  ...
  while (latitude == "0.00" || longitude == "0.00"){
    getLocation();
    delay(1000);
  }
...
void getLocation() {
  if (WiFi.status() == WL_CONNECTED) {
    ...
    if (httpResponseCode > 0) {
      ...
      Serial.println(payload); // Konum verilerini seri monitöre yazdır

      // JSON verilerini ayrıştırma işlemi
      ...
      // JSON'dan latitude ve longitude değerlerini al
